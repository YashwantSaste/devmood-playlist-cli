...
const { signup, login } = require('../auth/authManger.js'); // Import signup and login functions
...
program.command('signup')
  ...
  .action(async () => {
    ...
    try {
      const userCredential = await signup(answers.email, answers.password);
      console.log("userCredential:", userCredential);  // Check the returned object
      logSuccess(`Signup successful! Welcome, ${userCredential.email}`);
    } catch (error) {
...
program.command('login')
  .description('Login with your Firebase account')
  .action(async () => {
    const answers = await inquirer.prompt([
      { name: 'email', message: 'Email:' },
      { name: 'password', type: 'password', message: 'Password:' }
    ...
    try {
      const userCredential = await login(answers.email, answers.password);
      logSuccess(`Login successful! Hello again, ${userCredential.user.email}`);
    } catch (error) {
